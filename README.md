...
- 类型：目前可以选择llm、vlm和embedding。llm类型可用于[LLM节点](#llm节点)、[ReactAgent](#reactagent)、[SupervisorAgent](#supervisoragent)等，vlm类型可用于[VLM节点](#vlm节点)，embedding类型可用于[向量召回节点](#向量召回节点)。
...
在向量库配置中，点击【📖】标签，进入向量库文档管理页面

点击右上角的【📥】图标可以添加文档

点击【🔄】图标可以将文档向量化后存入向量数据库
...
结束节点是Agent的终点，表示该Agent执行完毕，双击结束节点会弹出侧边栏展示Agent的运行结果。
...
即多模态模型节点，可以通过提示词来对图片进行识别、描述等操作。参考用例[examples/vlm_demo.json]()

- 名称：必要，需要保证在Agent中唯一
- 模型：必要，可选择[模型配置](#模型配置)中的llm模型
- 系统提示词：可选，系统提示词。系统提示词可以使用[状态变量](#state)，语法请参考[状态变量使用规则](#状态变量使用规则)。
- 用户提示词：可选，用户提示词。用户提示词可以使用状态变量，语法请参考[状态变量使用规则](#状态变量使用规则)。
- 图片URL：必要，需要进行处理的图片的URL，也可以使用[图片加载节点](#图片加载节点)的输出，可以使用状态变量，语法请参考[状态变量使用规则](#状态变量使用规则)。
...
可以上传txt、pdf、docx、md格式的文档，输出文档内容。参考用例[examples/doc_demo.json]()
...
可以上传jpg、png、jpeg格式的图片，输出图片的base64编码。参考用例[examples/vlm_demo.json]()

- 名称：必要，需要保证在Agent中唯一
- 引导词：可选，上传图片前给用户的提示信息

##### 文档保存节点

将大语言模型的输出保存到文件。参考用例[examples/doc_demo.json]()
...
可以执行当前操作系统的命令，输出命令执行结果。参考用例[examples/command_demo.json]()

- 名称：必要，需要保证在Agent中唯一
- 执行命令：必要，在节点中执行的命令
- 命令执行目录：可选，切换命令执行的目录，如果不填写，则默认在lang-agent-backend目录下执行
