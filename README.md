...
- URI：必要，向量数据库的URI
- 用户名：可选，向量数据库的用户名(建议将用户名配置为环境变量PGVECTOR_USER或MILVUS_USER)
- 密码：可选，向量数据库的密码(建议将密码配置为环境变量PGVECTOR_PASSWORD或MILVUS_PASSWORD)
- 数据库名：必要，向量数据库的名称
- 集合名：必要，向量数据库的集合名称
- 嵌入模型：必要，可选择的嵌入模型，即[模型配置](#模型配置)中的embedding模型
- 文本内容：必要，需要进行向量化存储的文本内容。可以使用[状态变量](#state)，语法请参考[状态变量使用规则](#状态变量使用规则)。
